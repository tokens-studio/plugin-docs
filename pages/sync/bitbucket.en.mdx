...
- `Save the URL of the repository` somewhere for safekeeping and plugin configuration later. 
...
When you created the `repository` from the steps above there was a step to `save the URL of the repository somewhere safe`. 
...
This setting also has some impact on teams using Tokens Studio where not all team members are on the pro licence or when you want a "read-only" option for certain team members
