...
Run `node token-transformer tokens.json output.json` where input.json is the file you stored your tokens in.This will take your input JSON file (which is the file we generate from Figma Tokens, this should match what you entered in the Sync details) and output the transformed tokens to an output.json
...
<Callout emoji="💡">
  A basic example that uses GitHub as a sync storage and uses GitHub Actions to automatically transform tokens on each push to css variables can be found [here](https://github.com/six7/figma-tokens-example). 
