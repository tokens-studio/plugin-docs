...
2. [Create a new reopsitory](https://github.com/new). Give it a name, make it private or public, and initialize it with a **README** (a crucial step).
3. In the `Settings` tab of your profile, sroll to the bottom to find `Developer Settings` whereafter you should find [Peronsal Access Tokens](https://github.com/settings/tokens).
...
- Specify a file path where your tokens should be stored, e.g. `data/tokens.kson` or just `tokens.json`
...
We have have now moved over from using Token Transformer to using our Custom Tranforms for [Style-dictionary](/transforming for development/style dictionary) to work with Design Tokens. 
