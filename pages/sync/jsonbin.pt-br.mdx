...
### Pausando sincronização

Por padrão, cada mudança de token aciona uma atualização em seu provedor de sincronização. Você pode pausar este comportamento desmarcando `Update Remote` (no menu suspenso inferior esquerdo do plugin).