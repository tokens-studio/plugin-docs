...
To get started with enabling Sync for your file, head to the `Sync` tab in the plugin. Choose `URL` to select a .json stored on your server as your Sync Provider.
...
If your server requires some sort of authentication you can store your Headers in the headers field. It's important that your .json stored on your server has `Access-Control-Allow-Origins` set to `*`, ask your engineers to help you out on that if you're stuck or [read up on it](https://gist.github.com/nixta/0b98d7975562bc31c4c9). The URL part is the complete URL that points to your .json
