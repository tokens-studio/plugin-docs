...
To use aliases in tokens, we use the following notation: `{spacing.sm}`. Alternatively, you can alos use the notation with the dollar sign, like this: `$spacing.sm`.

You can also select any available aliases while creating a token by typing `{` and then also typing a part of the alias name in the field, or by selecting available aliases from the dropdown.
