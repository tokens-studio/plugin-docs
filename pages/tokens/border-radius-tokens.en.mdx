...
### Single radius tokens
Define a radius token with one single value.

#### How to use
Create an Border radius token under the `Border radius` category by clicking on the `+` icon.  
Enter a name for your token and the value for your border radius.

Once selected, apply Spacing tokens by either left-clicking (all values will use this token) or right-click to specify which part you want to target (`All`, `Top Right`, `Top Left`, `Bottom Right` or `Bottom Left`)

### Multi-value radius tokens
