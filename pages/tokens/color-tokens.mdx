...
For example, you may want your `space.sm` token reference the `space.xs` token, but multiply its value by 2. To do that, you can write the following as the token's value: `$spacing.xs * 2` (note the space between token and value)
